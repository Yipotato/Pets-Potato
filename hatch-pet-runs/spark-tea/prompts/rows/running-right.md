Create a single horizontal sprite strip for the Codex app digital pet `spark-tea` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Simplify the three red line reference sketches into a compact chibi boba tea cup pet: rounded cup body, red-orange thick outline, black fill or very dark tea body, small face, pearl dots near the bottom, a star sparkle attached near the top right rim, and a tiny exclamation-mark-like side accent. Keep the identity readable at 192x208..
